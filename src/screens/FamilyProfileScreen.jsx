...
export default function FamilyProfileScreen({
  ...
  const formatDate = (dateText) =>
    language === "en"
      ? dateText
          .replace("២៣ កក្កដា ២០២៦", "23 Jul 2026")
          .replace("១៨ កក្កដា ២០២៦", "18 Jul 2026")
          .replace("១០ កក្កដា ២០២៦", "10 Jul 2026")
      : dateText;
  const reportType = language === "en" ? "Lab Result" : "លទ្ធផលពិសោធន៍";
  const prescriptionType = language === "en" ? "Prescription" : "វេជ្ជបញ្ជា";
  const reportDates =
    language === "en"
      ? ["23 Jul 2026", "18 Jul 2026", "10 Jul 2026"]
      : ["២៣ កក្កដា ២០២៦", "១៨ កក្កដា ២០២៦", "១០ កក្កដា ២០២៦"];

  const member =
    familyMembers.find((m) => m.id === selectedId) || familyMembers[0];
  if (!member) {
    return (
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-neutral-800 dark:text-white">
                    {t("ប្រវត្តិការពិសោធន៍", "Test History")}
                  </p>
                  <p className="text-xs text-neutral-400 dark:text-neutral-500">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-error-600 dark:text-error-400">
                    {t("លុបសមាជិក", "Delete Member")}
                  </p>
                  <p className="text-xs text-neutral-400 dark:text-neutral-500">
                    {t("Delete Member", "Delete Member")}
                  </p>
                </div>
              </button>
            </div>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-warning-800 dark:text-warning-300">
                {t("ការជូនដំណឹងថ្មី", "Latest Alerts")}
              </p>
              <p className="text-xs text-warning-600 dark:text-warning-400">
                {t("Latest Alerts", "Latest Alerts")}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <p className="text-sm text-warning-700 dark:text-warning-300">
                <span className="font-semibold">
                  {t("ជាតិស្ករ", "Blood glucose")}
                </span>{" "}
                {t("ខ្ពស់", "high")} (142 mg/dL) — {t("២៣ កក្កដា", "23 Jul")}
              </p>
            </div>
            <div className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 bg-warning-400 dark:bg-warning-500 rounded-full mt-1.5 shrink-0" />
              <p className="text-sm text-warning-700 dark:text-warning-300">
                <span className="font-semibold">
                  {t("អេម៉ូក្លូប៊ីន", "Hemoglobin")}
                </span>{" "}
                {t("ទាប", "low")} (11.2 g/dL) — {t("២៣ កក្កដា", "23 Jul")}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 px-6 pt-6 pb-24">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-neutral-800 dark:text-white">
            {t("ប្រវត្តិរបាយការណ៍", "Report History")}
          </h2>
          <button
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-neutral-800 dark:text-white">
                      {language === "en" ? report.typeEn : report.type}
                    </p>
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      ...
                      <span className="text-xs text-neutral-400 dark:text-neutral-500">
                        {report.findings} {t("ចំណុច", "findings")}
                      </span>
                      {report.alerts > 0 && (
                        <>
                          <span className="w-1 h-1 bg-neutral-200 dark:bg-slate-600 rounded-full" />
                          <span
                            className={`text-xs font-medium ${colors.icon}`}
                          >
                            {report.alerts} {t("ការជូនដំណឹង", "alerts")}
                          </span>
                        </>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-2xl">
            <div className="w-14 h-14 bg-error-50 dark:bg-error-900/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg
                className="w-7 h-7 text-error-500 dark:text-error-400"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
            ...
            <p className="text-sm text-neutral-500 dark:text-neutral-400 text-center mb-6">
              {t("តើអ្នកប្រាកដជាចង់លុប", "Are you sure you want to delete")}{" "}
              <span className="font-semibold text-neutral-900 dark:text-white">
                {member.name}
              </span>{" "}
              {t("ចេញពីអ្នកថែទាមរបស់អ្នកទេ?", "from your caregivers?")}
            </p>
