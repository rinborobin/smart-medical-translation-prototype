const statusColors = {
  normal: { bg: 'bg-success-50 dark:bg-success-900/20', border: 'border-success-100 dark:border-success-800', dot: 'bg-success-500 dark:bg-success-400', text: 'text-success-700 dark:text-success-300' },
  warning: { bg: 'bg-warning-50 dark:bg-warning-900/20', border: 'border-warning-100 dark:border-warning-800', dot: 'bg-warning-500 dark:bg-warning-400', text: 'text-warning-700 dark:text-warning-300' },
}

export default function CaregiverScreen({ onNavigate, familyMembers, onSelectMember }) {
  return (
    <div className="flex flex-col min-h-screen bg-neutral-50 dark:bg-slate-900 transition-colors">
      <div className="flex items-center justify-between px-6 pt-14 pb-4">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900 dark:text-white">អ្នកថែទាំ</h1>
          <p className="text-xs text-neutral-400 dark:text-neutral-500">Caregiver Dashboard</p>
        </div>
        <button
      ...
      <div className="px-6 pt-2 pb-4">
        <div className="bg-primary-50 dark:bg-primary-900/20 rounded-2xl p-4 flex items-center gap-3">
          <svg className="w-5 h-5 text-primary-600 dark:text-primary-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
          </svg>
          <div>
            <p className="text-sm font-medium text-primary-800 dark:text-primary-300">ការជូនដំណឹងថ្មី</p>
            <p className="text-xs text-primary-600 dark:text-primary-400">២ សមាជិកមានលទ្ធផលត្រូវយកចិត្តទុកដាក់</p>
          </div>
        </div>
      </div>

      <div className="flex-1 px-6 space-y-3 pb-24">
        <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">សមាជិកគ្រួសារ</p>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`w-14 h-14 ${colors.bg} rounded-2xl flex items-center justify-center shrink-0`}>
                  <span className={`text-xl font-bold ${colors.text}`}>{member.avatar}</span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neutral-400 dark:text-neutral-500 mb-3">{member.nameEn} · {member.relation}</p>

                  <div className="flex items-center gap-3">
                    <span className="text-xs text-neutral-400 dark:text-neutral-500">របាយការណ៍ចុងក្រោយ: {member.lastReport}</span>
                  </div>

                  {member.alerts > 0 && (
                    <div className="mt-3 flex items-center gap-2">
                      <svg className="w-4 h-4 text-warning-500 dark:text-warning-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                      </svg>
                      <span className="text-xs font-medium text-warning-700 dark:text-warning-300">
                        {member.alerts} ការជូនដំណឹង
                      </span>
                    </div>
                  )}
                </div>
        ...
        >
          ...
          <span className="text-sm font-medium text-neutral-500 dark:text-neutral-400">បន្ថែមសមាជិកគ្រួសារ</span>
        </button>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white dark:bg-slate-800 border-t border-neutral-100 dark:border-slate-700 px-6 pb-6 pt-3 transition-colors">
        <div className="flex items-center justify-around">
          <button onClick={() => onNavigate('home', { replace: true })} className="flex flex-col items-center gap-1">
